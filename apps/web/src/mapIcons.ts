...
// lucide "waves" and "video", in their 24 × 24 box
const GLYPHS = {
  flood: [
...
export const CLUSTER_COLOR: Record<Kind, string> = { flood: '#1565c0', camera: '#37474f' };
...
export function mapImage(name: string): MapImage | null {
  ...
  const match = /^cluster-(flood|camera)-(.+)$/.exec(name);
