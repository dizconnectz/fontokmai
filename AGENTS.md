...
> อัปเดตล่าสุด: 2026-09-25 21:54 ICT (Claude) · เวลาเป็น ICT (UTC+7) · วันที่แบบ ISO (ค.ศ.)
...
#### 2026-09-25 21:54 ICT — iTIC live และข้อมูลเปิดเหตุการณ์ iTIC/Longdo (Claude)
- ผู้ใช้แนะนำ `live.iticfoundation.org` · เป็นแผนที่จราจรบน Longdo Map ที่มีเหตุการณ์สด (รวมฝนตก/น้ำท่วมที่คนรายงาน) กล้องจราจร และฝุ่น · ไม่มีช่องทางอ่านข้อมูลสดแบบเปิด → ลิงก์ออกเท่านั้น (D27)
- **พบข้อมูลเปิด**: เหตุการณ์ย้อนหลัง 2012–2026 เป็น CSV รายปี CC BY 4.0 (`type` 5 = ฝนตก, 6 = น้ำท่วม) → ใช้ทำ “จุดที่เคยมีรายงานน้ำท่วม” และ label ตรวจความแม่น (design 7.9) · ไม่มีปี 2554
- **ไม่ใช้**: เรดาร์ RainViewer (ฟรีเฉพาะใช้ส่วนตัว/การศึกษา) · ไอเดียให้ผู้ใช้รายงานน้ำท่วมเองไว้หลัง P2 (design 3)
- **ไฟล์**: `docs/sources.md`, design (3, 7.9, 16), AGENTS.md (A6, D) · ไม่มีโค้ด
...
4. หลัง DXS อนุมัติ Claude ทำตัวเก็บ DXS และไฟล์ “ถนนน้ำท่วม” (ค้นชื่อถนน + ลิงก์ต้นทาง) · ระหว่างนี้ทำ “ถนนนี้เคยท่วมกี่ครั้ง” จากสถิติ CC BY ของ กทม. และเหตุการณ์น้ำท่วม iTIC/Longdo (CC BY 4.0) และประวัติสถานีนำร่องจากข้อมูลเปิด สสน. (ไม่ใช้ ThaiWater API ตาม D27)
