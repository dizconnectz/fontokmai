...
> อัปเดตล่าสุด: 2026-09-26 01:04 ICT (Claude) · เวลาเป็น ICT (UTC+7) · วันที่แบบ ISO (ค.ศ.)
...
3. **งานถัดไป (ผู้ใช้ขอ 2026-09-26, เรียงตามลำดับ)** · รายละเอียดใน design 12:
   - **สีตามความหมาย** ทั้งเว็บ: เขียว ปกติ → เหลือง เฝ้าระวัง → ส้ม เตือน → แดง รุนแรง · เทา ไม่มีข้อมูล · ประกาศใช้ `severity` ของ CAP · มีไอคอนและคำกำกับเสมอ
   - **ปักหมุด**: การ์ดสรุป ตอนนี้ / ฝน 7 วัน / น้ำท่วมแถวนี้ แล้วกดดูรายละเอียด · ตอนนี้ทำส่วน “ตอนนี้” กับ “ถนนใกล้หมุดที่เคยท่วม” ได้แล้ว
   - **ค้นหาถนนที่เคยท่วม** จาก `ref/road_flood_history.json` ตาม `contracts/v1/README.md` ข้อ 8 · ทดสอบกับ `examples/road-flood-history/expected.json` (ส่วน `normalize`, `searches`, `near`)
   - `apps/web`: Claude แก้ base path สำหรับ GitHub Pages และให้ vite plugin เสิร์ฟไฟล์ JSON ทุกไฟล์ใน examples (ยกเว้น `expected.json`) โปรดรีวิว

## D. ขั้นต่อไป
1. (เสร็จ) P0-A, P0-B1, เว็บ P0 บน GitHub Pages (D28) และ `ref/road_flood_history.json`
2. Codex: งานใน C2 ข้อ 3 (สี, ปักหมุด, ค้นถนน)
3. Claude: ข้อมูลฝน · (ก) ภาพเรดาร์รวมกรมอุตุฯ เป็นชั้น “ฝนตกตอนนี้” (ข) พยากรณ์ 7 วันจาก Open-Meteo รายตำบลสำหรับการ์ดปักหมุด
4. Claude: P0-B2 ตัวคุมดิสก์ + retention + สำรอง SQLite (ต้องเสร็จก่อนเพิ่มแหล่งใหญ่), รายงาน slot, ตัวตรวจนอกเครื่อง, ซ้อม takeover/restore
5. ผู้ใช้สมัคร DXS (C1) → Claude ทำตัวเก็บ DXS (ระดับน้ำคลอง ฝน รายงานน้ำท่วมขังรายวันของ กทม.)
6. Claude สำรวจระบบโทรมาตรกรมทรัพยากรน้ำ และหาระดับสูงสุดปี 2554 ของสถานีนำร่อง
