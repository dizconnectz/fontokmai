...
> อัปเดตล่าสุด: 2026-09-25 21:50 ICT (Claude) · เวลาเป็น ICT (UTC+7) · วันที่แบบ ISO (ค.ศ.)
...
#### 2026-09-25 21:50 ICT — D27: ขอข้อมูลเฉพาะช่องทางที่เปิดรับ และนัดสมัคร DXS (Claude)
- **ผู้ใช้ตัดสิน (D27)**: ไม่ส่งจดหมายขอข้อมูลแบบทั่วไป ขอเฉพาะช่องทางที่หน่วยงานเปิดรับจริง → ไม่ขอ สสน. (ไม่มีช่องสมัคร), ไม่ขอเซนเซอร์สด กทม. (ไม่อยู่ใน DXS) และไม่ขอ iaunn · แหล่งเหล่านี้ใช้ได้แค่ลิงก์ออก หรือข้อมูลเปิดที่มี license
- **นัด 2026-09-26**: ผู้ใช้สมัคร DXS แบบประชาชนทั่วไป วัตถุประสงค์ “เผยแพร่ต่อ” ตามขั้นตอนใน `docs/requests/2026-09-26-bma-dxs.md` (บริการที่ใช้, วิธีส่ง key ขึ้น VPS โดยไม่ผ่านแชท, สรุปเงื่อนไข)
- **Codex**: ข้อมูล กทม. จะมาทาง DXS หลังอนุมัติ · ห้ามดึงหน้าเว็บ กทม. หรือ ThaiWater จากฝั่งเว็บ · เครดิตเป็นข้อความ ไม่ใช้โลโก้หน่วยงาน
- **ไฟล์**: `docs/requests/2026-09-26-bma-dxs.md` (ใหม่), `docs/sources.md`, design (0, 2, 3, 5, 7.4, 15), AGENTS.md (B, C1, D) · ไม่มีโค้ด
- **ขั้นต่อไป**: Claude ทำ “ถนนนี้เคยท่วมกี่ครั้ง” จากสถิติ CC BY และตัวคุมดิสก์ (P0-B2) ระหว่างรอ DXS
...
4. หลัง DXS อนุมัติ Claude ทำตัวเก็บ DXS และไฟล์ “ถนนน้ำท่วม” (ค้นชื่อถนน + ลิงก์ต้นทาง) · ระหว่างนี้ทำ “ถนนนี้เคยท่วมกี่ครั้ง” จากสถิติ CC BY ของ กทม. และประวัติสถานีนำร่องจากข้อมูลเปิด สสน. (ไม่ใช้ ThaiWater API ตาม D27)
