# fontokmai (ฝนตกไหม) — เอกสารออกแบบฉบับรวม v6

> **สถานะ**: v6 = v5 ที่ปรับตามรีวิว v5 ของ Codex (`private/handoffs/2026-09-25-codex-v5-review.md`) · อัปเดต 2026-09-25 (Claude) · D26 ตกลงแล้วตามเงื่อนไขข้อ 4.3–4.5 และ 4.9 · **รอผู้ใช้สั่งเริ่ม P0** ส่วน Codex ตรวจ v6 ได้คู่ขนานโดยไม่บล็อก P0 ขั้นแรก
...
5. **Claude — ThaiWater คู่ขนาน**: adapter + fixtures และเก็บ archive แบบ private (ฝน, เซนเซอร์น้ำท่วมถนน, ระดับน้ำคลอง) เพื่อไม่เสียข้อมูลฤดูฝน แต่ยังไม่เผยแพร่จนกว่าจะได้สิทธิ์จาก สสน. (ผู้ใช้ส่งคำขอ Claude ร่างให้) และห้ามเผยแพร่เพียงเพราะลบชื่อแหล่ง
...
11. **ช่วงพัฒนา** เริ่มบน GitHub Pages ได้ก่อน (ใช้บัญชีเดียว)
...
- **สำเนานอกเครื่อง** วันละครั้งไปที่ private GitHub repo ในรูป Releases (ไฟล์ละต่ำกว่า 2 GiB)
  ...
  - ไม่มี secrets · ข้อมูลที่สิทธิ์เผยแพร่ยังไม่ชัดอยู่ได้เฉพาะใน private repo ห้ามเข้า repo สาธารณะ และส่วนใดที่สำรองนอกเครื่องไม่ได้ต้องระบุว่ากู้ไม่ได้เมื่อเสียทั้งเครื่อง
...
| พยากรณ์ที่ archive | พื้นที่นำร่องเต็ม 90 วัน · ทั่วประเทศเก็บเฉพาะสรุปรายวัน | ถาวร |
...
- **ฝนและน้ำที่วัดได้**: ThaiWater (สาธารณะแต่ไม่มีเอกสารทางการ และสิทธิ์ยังไม่ชัด ต้องขออนุญาต สสน. ก่อนเผยแพร่), RID Reservoir API, TMD Open API และ RSS รายงานอากาศรายสถานี
...
- **ข้อมูลที่สิทธิ์ยังไม่ชัด** (ThaiWater, RID, GISTDA, ขอบเขตการปกครองบางชุด) ต้องมีตาราง “ฟีเจอร์ใดพึ่งแหล่งใดและ fallback คืออะไร” ใน P0 และขอสิทธิ์อย่างเป็นทางการ การระบุว่า license ไม่ชัดไม่ได้แก้เรื่องสิทธิ์
...
- `F` = ฝนพยากรณ์รายชั่วโมงของโซน = median ของโมเดลที่ผ่าน QC และสด โดย TMD WRF 2 กม. นับเป็นสมาชิกหนึ่งตัวเมื่อมี (ไม่นับซ้ำ) · ≥ 2 โมเดล = ครบ · 1 โมเดล = ใช้ได้แต่ `data_status=insufficient` และติดป้าย “แหล่งเดียว” · 0 โมเดล = unknown · TMD ขาดไม่ทำให้เป็น unknown ถ้ายังมีโมเดลอื่นครบเงื่อนไข
...
- **ความสด**: `age = t_eval − observed_at` (โมเดลใช้เวลารอบ) · สดเมื่อ `age ≤ อายุสูงสุด` · เวลาในอนาคตเกิน 5 นาที = QC ไม่ผ่าน · โมเดลที่ไม่ทราบรอบใช้ `available_at` กับอายุสูงสุด 12 ชม. และติด `run_unknown` (ห้ามใช้เวลาดาวน์โหลดแทนเวลารอบ) · ถ้าช่วงฝนที่ต้องใช้ขาดเกิน 1 ชม. เงื่อนไขนั้นเป็น unknown
- **Hysteresis**: ขึ้นป้ายเมื่อเงื่อนไขเป็น true · ลดป้ายเมื่อเป็น false ติดกัน 2 รอบประเมิน (≥ 30 นาที) ด้วยข้อมูลสด · ข้อมูลค้างให้เป็น `unknown` และแสดงผลครั้งก่อนแยกพร้อมเวลา ไม่คงป้ายเดิม
...
| `canal_reach` × `canal_overflow` | ตอนนี้ (observation ณ สถานี) | `P > 70` | `P ≥ 100` แสดงเป็น “ระดับน้ำเกินตลิ่งอ้างอิงที่สถานี…” | `P` = ระดับน้ำเทียบตลิ่งอ้างอิงตามนิยามของผู้ให้ข้อมูล (บันทึก field และ datum) ไม่สมมติว่าทุกสถานีคำนวณแบบเดียวกัน · scope เป็นจุดวัด ไม่ยืนยัน polygon ท่วม · ไม่มีตลิ่ง/datum หรือเก่ากว่า 60 นาที → unknown |
| `canal_reach` × `canal_overflow` | 0–6 ชม. | `P > 70` | ((`P ≥ 90` AND `S ≥ 5`) OR (`P ≥ 80` AND `Fsum_catch(0–6) ≥ 35`)) AND `G` | `S` = อัตราการขึ้น (ซม./ชม.) จาก linear regression ของ ≥ 3 ค่าในช่วง 2 ชม. ล่าสุด ช่องว่างไม่เกิน 40 นาที และ datum เดียวกัน ไม่เช่นนั้น unknown · `Fsum_catch` = ฝนพยากรณ์ 6 ชม. ของพื้นที่ระบายของคลอง (พารามิเตอร์ทดลอง) · `G` = ทราบการเดินประตู/ปั๊มที่เกี่ยวข้อง ตอนนี้เป็น unknown จึงได้สูงสุด `watch` พร้อม `may_be_higher=true` และเหตุผล ไม่ใช่เพดานที่บอกว่าประเมินครบแล้ว |
...
|---|---|
| สิทธิ์ข้อมูลที่ P1 ต้องใช้ยังไม่ชัด (ThaiWater, RID, GISTDA, ขอบเขต) | ตารางฟีเจอร์ → แหล่ง → fallback ใน P0 และขอสิทธิ์ทางการ |
...
| สสน. ไม่ให้สิทธิ์เผยแพร่ข้อมูล ThaiWater | เก็บแบบ private ต่อได้ ใช้แหล่งทางการอื่นเท่าที่มี และลดสิ่งที่แสดงบนเว็บ |
