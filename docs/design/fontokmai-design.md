...
- **D28 (ผู้ใช้ตัดสิน)**: เว็บขึ้น GitHub Pages ที่ `dizconnectz.github.io/fontokmai` แทน Cloudflare Pages
...
| ชื่อ | fontokmai (ฝนตกไหม) → `https://dizconnectz.github.io/fontokmai/` (D28) และ repo `github.com/dizconnectz/fontokmai` |
...
9. **Host (เปลี่ยนตาม D28)**: ผู้ใช้เลือก GitHub Pages ทั้งเว็บและข้อมูล ข้อ 9–10 และ 15 เดิมเก็บไว้เป็นทางเลือกเมื่อผู้ใช้มากขึ้น · เดิม: Cloudflare Pages แบบ Direct Upload จาก VPS โดย P0 ต้องตรวจโควตาจริงของบัญชีฟรีสำหรับ 96 ครั้ง/วัน (2,880 ครั้ง/30 วัน) ก่อนใช้ต่อเนื่อง การผ่าน 7 วันยังไม่พิสูจน์ว่าทั้งเดือนไม่ติดโควตา
...
/data/v1/ref/road_flood_history.json ประวัติน้ำท่วมถนน กทม.–ปริมณฑล (มีแล้ว สัญญาข้อ 8, สร้างใหม่รายสัปดาห์)
...
- **สีตามความหมาย** (ผู้ใช้ขอ 2026-09-26: ตอนนี้ทั้งเว็บโทนเดียวกัน อ่านยาก):
  - เขียว = ปกติหรืออากาศดี · เหลือง = เฝ้าระวัง · ส้ม = เตือน · แดง = รุนแรงหรืออันตราย · เทา = ไม่มีข้อมูลหรือข้อมูลเก่า
  - ฝนใช้สเกลสีฟ้าแยก (เบาไปหนัก) เพื่อไม่ปนกับสีระดับอันตราย · สีแบรนด์ใช้เฉพาะหัวเว็บและปุ่ม ไม่ใช้บอกสถานะ
  - ประกาศ CAP ใช้ `severity`: Extreme = แดง, Severe = ส้ม, Moderate = เหลือง, Minor = เหลืองอ่อน, Unknown = เทา และค่าที่ระบบประเมินเองใช้สีเดียวกันแต่ติดป้าย “ทดลอง”
  - สีต้องตรงกันทุกที่ (แผนที่ รายการ การ์ด legend) และมีไอคอนกับคำกำกับเสมอ ไม่ใช้สีอย่างเดียว (คนตาบอดสี, WCAG)
- **ปักหมุดเป็นหน้าแรกของการใช้งาน** (ผู้ใช้ขอ 2026-09-26 พร้อมภาพตัวอย่างในแชท):
  - แตะค้างบนแผนที่, ค้นตำบล/ถนน/สถานที่ หรือใช้ตำแหน่งปัจจุบัน (ขออนุญาตก่อน และไม่ส่งพิกัดออกจากเครื่อง)
  - การ์ดสรุปสั้น 3 ส่วน: “ตอนนี้” (ประกาศที่ครอบจุดนั้น) → “ฝน 7 วัน” (ไอคอนและโอกาสฝนรายวันพร้อมสรุปหนึ่งบรรทัด) → “น้ำท่วมแถวนี้” (รายงานตอนนี้และถนนใกล้หมุดที่เคยท่วม)
  - รายละเอียดเปิดเมื่อกด “ดูรายละเอียด” เช่น ฝนรายชั่วโมง ค่ากลางและช่วงความต่างของโมเดล ประกาศเต็ม และรายงานน้ำท่วม
  - บันทึกหมุดไว้ในเครื่อง (ไม่ต้องมีบัญชี) · ทุกค่าบอกที่มาและเวลา · ส่วนที่ยังไม่มีข้อมูลแสดง “ยังไม่มีข้อมูล” ไม่ซ่อน
  - ข้อมูลที่ใช้ได้แล้ว: ประกาศ CAP และ `ref/road_flood_history.json` (ถนนใกล้หมุด) · ฝน 7 วันรอไฟล์พยากรณ์ (Claude)
  - **ช่องค้นหาบนสุด** (ผู้ใช้ขอ 2026-09-26): ค้นตำบล/อำเภอ/จังหวัดจาก `ref/places.json` (กรมการปกครอง) ในเครื่อง, ถนนที่เคยท่วม และสถานที่อื่นจาก Photon (OpenStreetMap, ส่งเฉพาะคำที่พิมพ์) → เลือกแล้วแผนที่ซูมให้เห็นทั้งพื้นที่ ปักหมุดพร้อมป้ายชื่อ และแถบข้างแสดงสรุปของพื้นที่นั้น · กติกาค้นอยู่ในสัญญาข้อ 11
  - **แถบเลื่อนเวลาใต้แผนที่** (ผู้ใช้ขอ 2026-09-26): ย้อนหลัง 1 ชม. เป็นเรดาร์ (สังเกตจริง) → ตอนนี้ → ล่วงหน้า 72 ชม. เป็นพยากรณ์ฝนรายชั่วโมง (Open-Meteo, สัญญาข้อ 12, ป้ายสีม่วง “พยากรณ์”) · เลื่อนแล้วแผนที่เปลี่ยนตามเวลานั้น: สีฝน, ขอบเขตประกาศที่มีผล ณ เวลานั้น (เฉพาะที่ออกแล้ว) และการ์ดหมุดอ่านค่าพยากรณ์ของชั่วโมงนั้น · ปุ่มเล่นต่อเนื่อง และปุ่มกลับมาตอนนี้ · การ์ดหมุดมีฝน 7 วัน (ฝนรวมทั้งวัน โอกาสฝน ไอคอนอากาศ)
  - แผนที่ต้องไม่ขยับเองหลังผู้ใช้ซูมหรือเลื่อน: ซูมตามสิ่งที่เลือกได้ครั้งเดียวต่อการเลือก ไม่ใช่ทุกครั้งที่ข้อมูลอัปเดต (บั๊กที่ผู้ใช้เจอในมือถือ 2026-09-26)
  - **ข้อมูลที่เป็นจุดเป็นหมุด** (ผู้ใช้ขอ 2026-09-26 ตามตัวอย่าง namnai.onrender.com): หมุดหยดน้ำมีไอคอนและสีตามชนิด (รายงานน้ำท่วม กล้อง และต่อไปสถานีน้ำ/ฝนของ DXS) · จุดใกล้กันรวมเป็นวงกลมตัวเลข แตะเพื่อซูม · แตะหมุดหรือรายการในแถบข้างเปิด popup บนแผนที่ โดยแถบข้างไม่เปลี่ยน และข้อมูลของจุดนั้นเปิดเมื่อกดขอเท่านั้น · หมุดบน canvas กดด้วยคีย์บอร์ดไม่ได้ จึงต้องมีรายการในแถบข้างของทุกชั้นเป็นทางเข้าสำหรับคีย์บอร์ดและ screen reader
- **ชั้น “ฝนตกตอนนี้”**: ภาพเรดาร์รวมของกรมอุตุฯ (ข้อ 5, ใช้แบบไม่แสวงกำไรพร้อมเครดิต) ซ้อนบนแผนที่พร้อมเวลาภาพ และข้อความ “ภาพเรดาร์ ไม่ใช่ปริมาณฝนที่วัดได้” · ภาพเป็น Web Mercator (ตรวจจากสัดส่วน 1800×2644 px, รีวิว Codex M1 2026-09-26) ค่าที่หมุดต้องอ่านแถวตาม Mercator y ให้ตรงกับที่แผนที่วาด (สัญญาข้อ 9)
